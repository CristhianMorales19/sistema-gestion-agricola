...
if (!auth0Domain || !auth0Audience) {
  console.warn(
    "⚠️ AUTH0 variables not set, Auth0 verification will be skipped",
  );
}

if (!serverSecret) {
  console.warn(
    "⚠️ SERVER_JWT_SECRET not set — local DB login tokens will be disabled",
  );
}

const client = auth0Domain
  ? jwksClient({ jwksUri: `https://${auth0Domain}/.well-known/jwks.json` })
  : null;
...
async function verifyAuth0Token(token: string): Promise<JwtPayload> {
  ...
  const getSigningKey = promisify(client.getSigningKey) as (
    kid: string,
  ) => Promise<{
    publicKey?: string;
    rsaPublicKey?: string;
...
export const flexibleAuth = async (
  ...
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Internal auth error" });
  }
};
