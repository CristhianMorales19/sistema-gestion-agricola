...
export class UserRoleController {
  ...
  static async getUsers(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (name)
        searchQuery += searchQuery ? ` AND name:*${name}*` : `name:*${name}*`;

      // Obtener usuarios de Auth0
      const auth0Result = searchQuery
        ? await auth0ManagementService.searchUsers(
            searchQuery,
            Number(page),
            Number(perPage),
          )
      ...
      const usersWithRoles: UserWithRoles[] = await Promise.all(
        auth0Result.users.map(async (user) => {
          ...
          const roles = await auth0ManagementService.getUserRoles(
            user.user_id!,
          );
          ...
          if (user.user_id && user.email) {
            try {
              ...
              let localUser = await prisma.mot_usuario
                .findUnique({
              ...
              if (!localUser && user.email) {
                localUser = await prisma.mot_usuario
                  .findFirst({
                    where: {
              ...
              console.warn(
                `No se pudo obtener datos locales para usuario ${user.email}:`,
                error,
              );
            }
          }
      ...
      if (role) {
        filteredUsers = usersWithRoles.filter((userWithRoles) =>
          userWithRoles.roles.some(
            (r: Auth0Role) =>
              r.name && r.name.toLowerCase().includes(role.toLowerCase()),
          ),
        );
      }
      ...
      if (hasRole !== undefined) {
        const hasRoleBool =
          typeof hasRole === "string" ? hasRole === "true" : Boolean(hasRole);
        filteredUsers = usersWithRoles.filter((userWithRoles) =>
          hasRoleBool
  ...
  static async getUserById(req: Request, res: Response): Promise<void> {
    try {
      ...
      try {
        ...
        console.warn(
          `No se pudo obtener datos locales para usuario ${userId}:`,
          error,
        );
      }
   ...
   */
  static async getUsersWithoutRoles(
    req: Request,
    res: Response,
  ): Promise<void> {
    try {
