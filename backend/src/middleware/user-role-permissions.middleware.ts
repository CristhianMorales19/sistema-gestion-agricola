 ...
 */
export const requireRolePermission = (
 ...
 */
export const requireUserRoleManagementPermission = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    const adminPermissions = ["roles:assign", "users:manage", "admin:full"];
...
export const auditRoleManagement = (action: string) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      const localUser = (
        req as Request & { localUser?: { usuario_id: number } }
      ).localUser;
