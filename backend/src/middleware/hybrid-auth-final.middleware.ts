...
export const hybridAuthMiddleware = async (
  ...
  try {
    ...
    console.log(
      "🔍 Criterios de búsqueda: userSub O userEmail + estado ACTIVO/activo",
    );
    ...
    try {
      ...
      if (
        ...
        console.warn(
          "⚠️ Prisma P2022 detectado (columna ausente). Usando fallback SQL para búsqueda de usuario.",
        );
        try {
    ...
    if (!user) {
      console.warn(
        "⚠️ Usuario NO encontrado en BD con los criterios de búsqueda (userSub/email)",
      );
      console.log(
        "🔍 Intentando fallback: usar permisos presentes en el token Auth0 si existen",
      );

      // Si el token trae permisos (claims) podemos confiar en ellos en entornos híbridos
      const tokenPermissions =
        (req as any).auth?.permissions || (req as any).user?.permissions || [];
      if (Array.isArray(tokenPermissions) && tokenPermissions.length > 0) {
        console.log(
          "🔁 Fallback aplicado: permisos obtenidos del token:",
          tokenPermissions,
        );
        ...
        console.warn(
          "⚠️ Usuario no encontrado en BD pero token contiene permisos — continuando usando permisos del token",
        );
        return next();
      }
    }
    ...
    console.log(
      `✅ Usuario encontrado: ID ${user.usuario_id}, username: ${user.username}, estado: ${user.estado}`,
    );
    ...
    const permissions = (rolPermisosQuery as any[]).map(
      (item) => item.permiso_codigo,
    );
    ...
    let mergedPermissions = Array.from(
      new Set([...(permissions || []), ...(tokenPermissions || [])]),
    );
    ...
    if (mergedPermissions.length === 0) {
      try {
        ...
        console.log(
          "🔎 Fallback - rows sin filtro is_activo:",
          JSON.stringify(fallbackRows, null, 2),
        );
        if (Array.isArray(fallbackRows) && fallbackRows.length > 0) {
          const fallbackPerms = fallbackRows
            .map((r) => r.permiso_codigo)
            .filter(Boolean);
          mergedPermissions = Array.from(
            new Set([...fallbackPerms, ...mergedPermissions]),
          );
        }
      } catch (fbErr) {
    ...
    if (
      ...
      rol &&
      (rol.codigo === "ADMIN" || rol.codigo === AGROMANO_ROLES.ADMIN_AGROMANO)
    ) {
      console.warn(
        "⚠️ Permisos resultaron vacíos para rol ADMIN; aplicando fallback ROLE_PERMISSIONS[ADMIN_AGROMANO]",
      );
      mergedPermissions = ROLE_PERMISSIONS[AGROMANO_ROLES.ADMIN_AGROMANO] || [];
      console.log(
        "🔁 Permisos aplicados desde ROLE_PERMISSIONS:",
    ...
    console.log(
      `🎉 Usuario autenticado: ${user.username} | Rol: ${rol.codigo} | Permisos: ${mergedPermissions.length}`,
    );
