...
const getUserPermissionsFromRequest = (req: Request): string[] => {
  ...
  if (anyReq.auth) {
    if (
      Array.isArray(anyReq.auth.permissions) &&
      anyReq.auth.permissions.length > 0
    )
      return anyReq.auth.permissions;
    if (
      Array.isArray(anyReq.auth["https://agromano.com/permissions"]) &&
      anyReq.auth["https://agromano.com/permissions"].length > 0
    )
      return anyReq.auth["https://agromano.com/permissions"];
  ...
  if (anyReq.user) {
    if (
      Array.isArray(anyReq.user.permissions) &&
      anyReq.user.permissions.length > 0
    )
      return anyReq.user.permissions;
    if (
      Array.isArray(anyReq.user["https://agromano.com/permissions"]) &&
      anyReq.user["https://agromano.com/permissions"].length > 0
    )
      return anyReq.user["https://agromano.com/permissions"];
  ...
  if (
    Array.isArray(anyReq.userPermissions) &&
    anyReq.userPermissions.length > 0
  )
    return anyReq.userPermissions;
  if (
    Array.isArray(anyReq.auth0User?.permissions) &&
    anyReq.auth0User.permissions.length > 0
  )
    return anyReq.auth0User.permissions;
...
const getUserRolesFromRequest = (req: Request): string[] => {
  ...
  if (
    Array.isArray(anyReq.auth?.["https://agromano.com/roles"]) &&
    anyReq.auth["https://agromano.com/roles"].length > 0
  )
    return anyReq.auth["https://agromano.com/roles"];
  if (Array.isArray(anyReq.user?.roles) && anyReq.user.roles.length > 0)
    return anyReq.user.roles;
  if (typeof anyReq.user?.role === "string" && anyReq.user.role)
...
export const requirePermission = (permission: AgroManoPermission) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!userPermissions.includes(permission)) {
      console.warn(
        `RBAC: requirePermission FAILED -> required=${permission} userPermissions=${JSON.stringify(userPermissions)} auth=${JSON.stringify((req as any).auth)} user=${JSON.stringify((req as any).user)}`,
      );
...
export const requirePermissions = (permissions: AgroManoPermission[]) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!hasAllPermissions) {
      ...
      console.warn(
        `RBAC: requirePermissions FAILED -> required=${permissions.join(", ")} missing=${JSON.stringify(missingPermissions)} userPermissions=${JSON.stringify(userPermissions)} auth=${JSON.stringify((req as any).auth)} user=${JSON.stringify((req as any).user)}`,
      );
      return res.status(403).json({
        success: false,
...
export const requireAnyPermission = (permissions: AgroManoPermission[]) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!hasAnyPermission) {
      console.warn(
        `RBAC: requireAnyPermission FAILED -> requiredAny=${permissions.join(", ")} userPermissions=${JSON.stringify(userPermissions)} auth=${JSON.stringify((req as any).auth)} user=${JSON.stringify((req as any).user)}`,
      );
      return res.status(403).json({
        success: false,
...
export const requireRole = (role: keyof typeof AGROMANO_ROLES) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!userRoles.includes(requiredRole)) {
      console.warn(
        `RBAC: requireRole FAILED -> requiredRole=${requiredRole} userRoles=${JSON.stringify(userRoles)} auth=${JSON.stringify((req as any).auth)} user=${JSON.stringify((req as any).user)}`,
      );
      return res.status(403).json({
        success: false,
...
export const userHasPermission = (
  user: Express.Request["auth"],
  permission: AgroManoPermission,
): boolean => {
...
export const userHasRole = (
  user: Express.Request["auth"],
  role: keyof typeof AGROMANO_ROLES,
): boolean => {
