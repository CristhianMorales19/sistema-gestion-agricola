 ...
 */
router.get("/estadisticas", checkJwt, AusenciasController.getEstadisticas);

/**
 * DEBUG: Verificar documentos
 */
router.get("/debug/documentos", AusenciasController.debugDocumentos);
 ...
 * @access Requiere autenticación Auth0
 */
router.get("/", checkJwt, AusenciasController.getAll);
 ...
 * @access Requiere autenticación Auth0
 */
router.post("/", checkJwt, AusenciasController.create);

/**
 * RUTAS CON PARÁMETROS :id - ORDEN IMPORTANTE
 * Las rutas más específicas (con subrutas) deben ir ANTES que /:id genérico
 */

/**
 * @route GET /api/ausencias/:id/documento
 * @desc Descargar documento de respaldo (sin autenticación para permitir carga de imágenes)
 * @access Público (pero solo si el usuario tiene acceso a la ausencia)
 */
router.get(
  "/:id/documento",
  // Permitir acceso sin autenticación pero verificar en el controlador
  AusenciasController.descargarDocumento,
);
 ...
 * @access Requiere autenticación Auth0
 */
router.post("/:id/aprobar", checkJwt, AusenciasController.aprobar);
 ...
 * @access Requiere autenticación Auth0
 */
router.post("/:id/rechazar", checkJwt, AusenciasController.rechazar);
 ...
 * @access Requiere autenticación Auth0
 */
router.get("/:id", checkJwt, AusenciasController.getById);
 ...
 * @access Requiere autenticación Auth0
 */
router.put("/:id", checkJwt, AusenciasController.update);
 ...
 * @access Requiere autenticación Auth0
 */
router.delete("/:id", checkJwt, AusenciasController.delete);
