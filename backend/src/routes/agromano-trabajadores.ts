...
router.post(
  ...
  (req, res, next) => {
    console.log(
      "3. Después de hybridAuthMiddleware - User:",
      (req as any).user,
    );
    next();
  },
        ...
        await prisma.mot_info_laboral.create({
          data: {
            ...
            salario_bruto: salario_bruto
              ? parseFloat(String(salario_bruto))
              : null,
            rebajas_ccss: rebajas_ccss
              ? parseFloat(String(rebajas_ccss))
              : null,
            otras_rebajas: otras_rebajas
              ? parseFloat(String(otras_rebajas))
              : null,
            salario_por_hora: salario_por_hora
              ? parseFloat(String(salario_por_hora))
              : null,
            horas_ordinarias: horas_ordinarias
              ? parseFloat(String(horas_ordinarias))
              : null,
            horas_extras: horas_extras
              ? parseFloat(String(horas_extras))
              : null,
            horas_otras: horas_otras ? parseFloat(String(horas_otras)) : null,
            vacaciones_monto: vacaciones_monto
              ? parseFloat(String(vacaciones_monto))
              : null,
            incapacidad_monto: incapacidad_monto
              ? parseFloat(String(incapacidad_monto))
              : null,
            lactancia_monto: lactancia_monto
              ? parseFloat(String(lactancia_monto))
              : null,
    ...
    try {
      ...
      if (!cargo || !salario_base || !tipo_contrato) {
        return res.status(400).json({
          success: false,
          message:
            "Todos los campos son requeridos: cargo, salario_base, tipo_contrato",
        });
      }
      ...
      const laboralData = {
        ...
        cargo: cargo
          ...
          : existingInfo
            ? existingInfo.cargo
            : "Sin definir",
        fecha_ingreso_at: req.body.fecha_ingreso_at
          ? new Date(req.body.fecha_ingreso_at)
          : existingInfo
            ? existingInfo.fecha_ingreso_at
            : new Date(),
        tipo_contrato: tipo_contrato
          ? String(tipo_contrato)
          : existingInfo
            ? existingInfo.tipo_contrato
            : "no_definido",
        salario_base: salario_base
          ? parseFloat(String(salario_base))
          : existingInfo
            ? parseFloat(String(existingInfo.salario_base))
            : 0,
        codigo_nomina: req.body.codigo_nomina
          ? String(req.body.codigo_nomina)
          : existingInfo
            ? existingInfo.codigo_nomina
            : null,
        salario_bruto: req.body.salario_bruto
          ? parseFloat(String(req.body.salario_bruto))
          : existingInfo
            ? existingInfo.salario_bruto
            : null,
        rebajas_ccss: req.body.rebajas_ccss
          ? parseFloat(String(req.body.rebajas_ccss))
          : existingInfo
            ? existingInfo.rebajas_ccss
            : null,
        otras_rebajas: req.body.otras_rebajas
          ? parseFloat(String(req.body.otras_rebajas))
          : existingInfo
            ? existingInfo.otras_rebajas
            : null,
        salario_por_hora: req.body.salario_por_hora
          ? parseFloat(String(req.body.salario_por_hora))
          : existingInfo
            ? existingInfo.salario_por_hora
            : null,
        horas_ordinarias: req.body.horas_ordinarias
          ? parseFloat(String(req.body.horas_ordinarias))
          : existingInfo
            ? existingInfo.horas_ordinarias
            : null,
        horas_extras: req.body.horas_extras
          ? parseFloat(String(req.body.horas_extras))
          : existingInfo
            ? existingInfo.horas_extras
            : null,
        horas_otras: req.body.horas_otras
          ? parseFloat(String(req.body.horas_otras))
          : existingInfo
            ? existingInfo.horas_otras
            : null,
        vacaciones_monto: req.body.vacaciones_monto
          ? parseFloat(String(req.body.vacaciones_monto))
          : existingInfo
            ? existingInfo.vacaciones_monto
            : null,
        incapacidad_monto: req.body.incapacidad_monto
          ? parseFloat(String(req.body.incapacidad_monto))
          : existingInfo
            ? existingInfo.incapacidad_monto
            : null,
        lactancia_monto: req.body.lactancia_monto
          ? parseFloat(String(req.body.lactancia_monto))
          : existingInfo
            ? existingInfo.lactancia_monto
            : null,
        fecha_ultima_actualizacion_at: new Date(),
      ...
      res.status(500).json({
        success: false,
        message:
          "Error interno del servidor al actualizar la información laboral",
      });
    }
  },
);
