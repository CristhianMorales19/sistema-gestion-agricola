...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        "👤 Usuario Auth0 después de checkJwt:",
        (req as any).user?.sub,
      );
      ...
      console.log(
        "   - dbUser:",
        (req as any).user?.dbUser ? "✅ Encontrado" : "❌ No encontrado",
      );
      console.log(
        "   - permissions length:",
        ((req as any).user?.permissions || []).length,
      );
      ...
      try {
        ...
        const totalTrabajadores = await prisma.mom_trabajador
          .count({
            where: {
              is_activo: true,
            },
        ...
        const totalUsuarios = await prisma.mot_usuario
          .count({
            where: {
        ...
        const totalRoles = await prisma.mom_rol
          .count({
            where: {
              is_activo: true,
            },
        ...
        const totalPermisos = await prisma.mom_permiso
          .count({
            where: {
              is_activo: 1,
            },
      ...
      const recentActivities = [
        ...
        },
        {
          ...
          tipo: "dashboard",
          mensaje: isAdvanced
            ? "Dashboard avanzado cargado - Acceso completo"
            : "Dashboard básico cargado - Acceso limitado",
      ...
      if (anyErr && anyErr.code === "P2022") {
        console.warn(
          "Prisma P2022 detectado en dashboard.general, devolviendo datos por defecto",
        );
        return res.json({
          success: true,
