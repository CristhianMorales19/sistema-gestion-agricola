...
import { AgroManoUserSyncService } from '../../../authentication/application/services/agromano-user-sync.service';
  ...
  try {
    ...
    if (!documento_identidad || !nombre_completo || !fecha_nacimiento) {
      return res.status(400).json({
        ...
        message: 'Campos requeridos: documento_identidad, nombre_completo, fecha_nacimiento'
      });
    }
 ...
 */
router.post('/:id/invitar-auth0', requireAdmin, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
 ...
 */
router.put('/:id/vincular-usuario', requireAdmin, async (req: Request, res: Response) => {
  try {
    const user = (req as any).user;
    const { id } = req.params;
    ...
    if (!usuarioAVincular) {
      return res.status(404).json({
        ...
        message: 'Usuario no encontrado. Asegúrate de que el trabajador ya se registró en Auth0 e hizo login al menos una vez.'
      });
    }

    // Verificar que el usuario no esté vinculado a otro trabajador
    if (usuarioAVincular.trabajador_id && usuarioAVincular.trabajador_id !== Number(id)) {
      return res.status(409).json({
        success: false,
  ...
  try {
    ...
    res.json({
      ...
      data: trabajadores.map(t => ({
        ...
        usuario: t.mot_usuario && t.mot_usuario.length > 0 ? {
