...
import {
  ...
  requireAdmin
} from '../../infrastructure/middleware/agromano-auth.middleware';
import { PrismaClient } from '@prisma/client';
import { Auth0ManagementService } from '../../application/services/auth0-management.service';
 ...
 */
router.get('/roles-disponibles', requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/sincronizar-auth0', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      success: true,
      message: 'La sincronización ocurre automáticamente cuando cada usuario hace login',
 ...
 */
router.post('/crear-hibrido', requireAdmin, async (req: Request, res: Response) => {
  try {
    const { email, nombre, password, rol_id } = req.body;

    console.log('🔵 [CREAR-HIBRIDO] Iniciando creación de usuario híbrido:', { email, nombre, rol_id });
    ...
    console.log('✅ [CREAR-HIBRIDO] Usuario creado en Auth0:', auth0User.user_id);
