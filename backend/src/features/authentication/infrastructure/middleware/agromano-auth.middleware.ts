...
export const agroManoAuthMiddleware = async (
  ...
  try {
    ...
    // Intenta primero con 'nombre' y luego con 'codigo' (por compatibilidad)
    const permisosDB = localUser.mom_rol?.rel_mom_rol__mom_permiso?.map((rp) => 
      rp.mom_permiso?.nombre || rp.mom_permiso?.codigo
    ).filter((permiso): permiso is string => permiso !== undefined) || [];

    // También extraer permisos del token de Auth0 (si vienen en el claim permissions)
    const permisosAuth0 = (auth0User as any).permissions || [];
    
    // Combinar permisos de BD local + Auth0 (sin duplicados)
    const permisos = [...new Set([...permisosDB, ...permisosAuth0])];
    
    console.log(`🔑 Permisos DB: ${permisosDB.length}, Auth0: ${permisosAuth0.length}, Total: ${permisos.length}`);
    console.log(`🔑 Permisos combinados:`, permisos);
