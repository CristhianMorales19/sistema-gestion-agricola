...
const getUserPermissionsFromRequest = (req: Request): string[] => {
  ...
  if (req.auth) {
    if (Array.isArray(req.auth.permissions) && req.auth.permissions.length > 0)
      return req.auth.permissions;
    if (
      Array.isArray(req.auth["https://agromano.com/permissions"]) &&
      req.auth["https://agromano.com/permissions"].length > 0
    )
      return req.auth["https://agromano.com/permissions"];
  ...
  if (Array.isArray(req.userPermissions) && req.userPermissions.length > 0)
    return req.userPermissions;
  if (
    req.auth0User &&
    Array.isArray(req.auth0User.permissions) &&
    req.auth0User.permissions.length > 0
  )
    return req.auth0User.permissions;
...
const getUserRolesFromRequest = (req: Request): string[] => {
  ...
  if (
    req.auth &&
    Array.isArray(req.auth["https://agromano.com/roles"]) &&
    req.auth["https://agromano.com/roles"].length > 0
  )
    return req.auth["https://agromano.com/roles"];
  if (
    req.user &&
    typeof req.user === "object" &&
    "roles" in req.user &&
...
export const requirePermission = (permission: AgroManoPermission) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!userPermissions.includes(permission)) {
      console.warn(
        `RBAC: requirePermission FAILED -> required=${permission} userPermissions=${JSON.stringify(userPermissions)} auth=${JSON.stringify(req.auth)} user=${JSON.stringify(req.user)}`,
      );
...
export const requirePermissions = (permissions: AgroManoPermission[]) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!hasAllPermissions) {
      ...
      console.warn(
        `RBAC: requirePermissions FAILED -> required=${permissions.join(", ")} missing=${JSON.stringify(missingPermissions)} userPermissions=${JSON.stringify(userPermissions)} auth=${JSON.stringify(req.auth)} user=${JSON.stringify(req.user)}`,
      );
      return res.status(403).json({
        success: false,
...
export const requireAnyPermission = (permissions: AgroManoPermission[]) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!hasAnyPermission) {
      console.warn(
        `RBAC: requireAnyPermission FAILED -> requiredAny=${permissions.join(", ")} userPermissions=${JSON.stringify(userPermissions)} auth=${JSON.stringify(req.auth)} user=${JSON.stringify(req.user)}`,
      );
      return res.status(403).json({
        success: false,
...
export const requireRole = (role: keyof typeof AGROMANO_ROLES) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!userRoles.includes(requiredRole)) {
      console.warn(
        `RBAC: requireRole FAILED -> requiredRole=${requiredRole} userRoles=${JSON.stringify(userRoles)} auth=${JSON.stringify(req.auth)} user=${JSON.stringify(req.user)}`,
      );
      return res.status(403).json({
        success: false,
