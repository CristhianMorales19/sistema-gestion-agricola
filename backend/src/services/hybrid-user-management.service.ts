...
export class HybridUserManagementService {
      ...
      try {
        ...
        const usersWithLocalData = await Promise.all(
          auth0Result.users.map(async (auth0User) => {
            const localUser = await this.findLocalUserByAuth0Id(
              auth0User.user_id,
            );
            const roles = await this.auth0Service.getUserRoles(
              auth0User.user_id!,
            );
        ...
        console.error(
          "Error obteniendo usuarios de Auth0, usando BD local:",
          error,
        );
  ...
  async getRoles() {
    ...
    if (auth0Available) {
      try {
        ...
        console.error(
          "Error obteniendo roles de Auth0, usando BD local:",
          error,
        );
  ...
  async assignRoles(userId: string, roleIds: string[]) {
    ...
    if (auth0Available) {
      try {
        ...
        console.error(
          "Error asignando roles en Auth0, usando BD local:",
          error,
        );
   ...
   */
  private async syncUserRolesToDatabase(
    auth0UserId: string,
    roleIds: string[],
  ) {
    try {
