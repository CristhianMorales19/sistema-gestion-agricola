...
router.get("/trabajadores-activos", async (_req: Request, res: Response) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        error: "Error obteniendo trabajadores activos",
        detalle: err.message,
      });
  }
});
...
router.get("/entradas-hoy", async (_req: Request, res: Response) => {
  try {
    ...
    const trabajadorIds = Array.from(
      new Set(entradas.map((e) => e.trabajador_id)),
    );
    const trabajadores = await prisma.mom_trabajador.findMany({
    ...
    const data = entradas.map((e) => {
      ...
      return {
        ...
        horaSalida: e.hora_salida_at
          ? e.hora_salida_at.toISOString().substring(11, 19)
          : null,
    ...
    res
      ...
      .json({
        success: false,
        error: "Error obteniendo entradas de hoy",
        detalle: err.message,
      });
  }
});
...
router.get("/pendientes-salida", async (_req: Request, res: Response) => {
  try {
    ...
    const trabajadorIds = Array.from(
      new Set(pendientes.map((p) => p.trabajador_id)),
    );
    const trabajadores = await prisma.mom_trabajador.findMany({
      where: { trabajador_id: { in: trabajadorIds } },
      select: {
    ...
    res
      ...
      .json({
        success: false,
        error: "Error obteniendo pendientes de salida",
        detalle: err.message,
      });
  }
});

interface RegistrarSalidaBody {
  trabajadorId: number;
  horaSalida?: string;
  observacion?: string;
}
...
router.post("/salida", async (req: Request, res: Response) => {
  try {
    const { trabajadorId, horaSalida, observacion }: RegistrarSalidaBody =
      req.body || {};
    if (!trabajadorId)
    ...
    console.log("[ASISTENCIA][SALIDA] Request =>", {
      trabajadorId,
      horaSalida,
      observacion,
      fechaInicio,
    ...
    const asistencia = await prisma.mot_asistencia.findFirst({
      where: {
        ...
        fecha_at: { gte: fechaInicio, lt: fechaFin },
        hora_salida_at: null,
      },
    });

    if (!asistencia) {
      console.warn("[ASISTENCIA][SALIDA] No encontrada asistencia abierta", {
        trabajadorId,
      });
      return res
        .status(404)
        .json({
          error: "No existe registro de entrada pendiente de salida para hoy",
        });
    }
    if (asistencia.hora_salida_at) {
      console.warn(
        "[ASISTENCIA][SALIDA] Ya tenía hora_salida_at",
        asistencia.asistencia_id,
      );
      return res
        .status(409)
        .json({ error: "Ya se registró la salida para este trabajador hoy" });
    }

    const baseFecha = asistencia.fecha_at.toISOString().slice(0, 10);
    const horaSalidaDate = horaSalida
      ? new Date(
          `${baseFecha}T${horaSalida.length === 5 ? horaSalida + ":00" : horaSalida}`,
        )
      : new Date();

    if (horaSalidaDate < asistencia.hora_entrada_at) {
      console.warn("[ASISTENCIA][SALIDA] Hora salida anterior a entrada", {
        horaSalidaDate,
        horaEntrada: asistencia.hora_entrada_at,
      });
      return res
        .status(400)
        .json({
    ...
    const diffMs =
      horaSalidaDate.getTime() - asistencia.hora_entrada_at.getTime();
    const horasTrab = Math.round((diffMs / (1000 * 60 * 60)) * 100) / 100; // 2 decimales

    console.log("[ASISTENCIA][SALIDA] Calculado =>", {
      asistenciaId: asistencia.asistencia_id,
      horasTrab,
      diffMs,
      horaEntrada: asistencia.hora_entrada_at,
      horaSalidaDate,
    ...
    res
      .status(500)
      .json({ error: "Error al registrar salida", detalle: err.message });
  }
});
...
router.post("/entrada", async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      "[ASISTENCIA][ENTRADA] Entrada registrada exitosamente =>",
      asistencia,
    );
