...
export class UserManagementService {
  ...
  async assignRoles(userId: string, roleIds: string[]): Promise<UserWithRoles> {
    const response = await this.request<any>(`/admin/users/${userId}/roles`, {
      method: 'PUT',
  ...
  async removeRole(userId: string, roleId: string): Promise<UserWithRoles> {
    const response = await this.request<any>(`/admin/users/${userId}/roles/${roleId}`, {
      method: 'DELETE',
    });
    return response.data || response;
  }
  ...
  async getRoles(): Promise<Role[]> {
    const response = await this.request<any>('/admin/roles');
