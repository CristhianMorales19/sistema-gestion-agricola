...
export const WorkerSelect: React.FC<WorkerSelectProps> = ({
    ...
    service
      ...
      .then((result) => {
        if (!Array.isArray(result)) {
          console.warn(
            "[WorkerSelect] Resultado inesperado, se esperaba array",
            result,
          );
      ...
      .catch((err) =>
        console.warn("[WorkerSelect] Error cargando trabajadores", err),
      )
      .finally(() => {
  ...
  return (
    <Autocomplete
      value={current}
      onChange={(_e, nuevo) =>
        onChange(nuevo ? nuevo.trabajador_id : null, nuevo || null)
      }
      inputValue={inputValue}
      onInputChange={(_e, nuevo) => setInputValue(nuevo)}
      options={options}
      getOptionLabel={(o) =>
        `${o.documento_identidad || o.trabajador_id} - ${o.nombre_completo}`
      }
      loading={loading}
      onKeyDown={(e) => {
        if (e.key === "Enter") {
          const exact = options.find(
            (o) =>
              o.documento_identidad === inputValue ||
              String(o.trabajador_id) === inputValue,
          );
          if (exact) {
            e.preventDefault();
