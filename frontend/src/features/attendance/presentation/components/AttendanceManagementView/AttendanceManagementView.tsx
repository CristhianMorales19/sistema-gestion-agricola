...
export const AttendanceManagementView: React.FC = () => {
  ...
  const [showBulkExitConfirmation, setShowBulkExitConfirmation] = useState(false);
  ...
  const [showBulkEntryConfirmation, setShowBulkEntryConfirmation] = useState(false);
  ...
  const registerExit = async (data: any, dateContext?: string): Promise<void> => {
    ...
    const attendance = attendancesArray.find(
      (a) => a.trabajador_id === data.trabajador_id && 
             getDateString(a.fecha_at) === dateToSearch &&
             a.hora_entrada_at
    );
    
    if (!attendance) {
      throw new Error('No se puede registrar la hora de salida sin antes registrar la hora de entrada');
    }
  ...
  const handleBulkEntrySelected = async (time: string, location: string) => {
    try {
      ...
      const registered: Array<{ trabajador_id: number; asistenciaId?: number }> = [];
      const toUpdate: Array<{ trabajador_id: number; asistenciaId: number; isReactivation: boolean }> = [];

      console.log(`📥 ENTRADA - Categorizando ${selectedWorkersList.length} trabajadores para ${dateToCheck}`);
      ...
      for (const worker of selectedWorkersList) {
        ...
        console.log(`👤 ${worker.nombre_completo}: ${allRecordsForWorker.length} registro(s) encontrado(s)`, 
          allRecordsForWorker.map(r => ({ id: r.asistencia_id, entrada: r.hora_entrada_at, deleted: !!r.deleted_at })));
        ...
        if (activeAttendance) {
          ...
          if (!activeAttendance.hora_entrada_at) {
            ...
            console.log(`  🔄 Actualizar entrada existente (ID: ${activeAttendance.asistencia_id})`);
            toUpdate.push({ trabajador_id: worker.trabajador_id, asistenciaId: activeAttendance.asistencia_id, isReactivation: false });
          }
          ...
          console.log(`  🔄 Reactivar desde ausencia (ID: ${deletedRecord.asistencia_id})`);
          toUpdate.push({ trabajador_id: worker.trabajador_id, asistenciaId: deletedRecord.asistencia_id, isReactivation: true });
        } else {
      ...
      console.log(`📊 RESUMEN: ${registered.length} nuevas, ${toUpdate.length} actualizaciones`);
  ...
  const processBulkEntry = async (
    ...
    console.log(`📊 [BULK ENTRY] Procesando: ${registered.length} nuevas, ${toUpdate.length} actualizaciones`);
    ...
    for (const worker of registered) {
      ...
      try {
        console.log(`📝 Registrando entrada para ${worker.nombre_completo} a las ${time}`);
        ...
        console.error(`❌ Error registrando ${worker?.nombre_completo}:`, error);
        errorCount++;
      }
    }
    ...
    for (const worker of toUpdate) {
      ...
      try {
        ...
        if (allRecords.length === 0) {
          console.error(`❌ No se encontró registro para ${worker.nombre_completo} en ${dateToCheck}`);
          errorCount++;
          continue;
        }
        ...
        console.log(`🔄 ${action} entrada para ${worker.nombre_completo} a las ${time} (ID: ${recordToUpdate.asistencia_id})`);
        ...
        console.error(`❌ Error actualizando ${worker?.nombre_completo}:`, error);
        errorCount++;
      }
    }
    ...
    if (registeredCount > 0) {
      resultParts.push(`✅ ${registeredCount} ${registeredCount === 1 ? 'entrada registrada' : 'entradas registradas'}`);
    }
    
    if (updatedCount > 0) {
      resultParts.push(`🔄 ${updatedCount} ${updatedCount === 1 ? 'entrada actualizada' : 'entradas actualizadas'}`);
    }
    
    if (errorCount > 0) {
      resultParts.push(`⚠️ ${errorCount} ${errorCount === 1 ? 'error' : 'errores'}`);
    }
  ...
  const processBulkExit = async (
    ...
    console.log(`📊 [BULK EXIT] Procesando: ${registered.length} nuevas, ${toUpdate.length} actualizaciones, ${rejectedCount} rechazadas`);
    ...
    for (const worker of registered) {
      ...
      try {
        console.log(`📝 Registrando salida para ${worker.nombre_completo} a las ${time}`);
        await registerExit({
          trabajador_id: worker.trabajador_id,
          horaSalida: time,
    ...
    for (const worker of toUpdate) {
      ...
      try {
        ...
        if (!activeRecord) {
          console.error(`❌ No se pudo encontrar registro activo para ${worker.nombre_completo}`);
          errorCount++;
          continue;
        }
        
        console.log(`🔄 Actualizando salida para ${worker.nombre_completo} a las ${time} (ID: ${activeRecord.asistencia_id})`);
    ...
    if (registeredCount > 0) {
      resultParts.push(`✅ ${registeredCount} ${registeredCount === 1 ? 'salida registrada' : 'salidas registradas'}`);
    }
    
    if (updatedCount > 0) {
      resultParts.push(`🔄 ${updatedCount} ${updatedCount === 1 ? 'salida actualizada' : 'salidas actualizadas'}`);
    }
    
    if (rejectedCount > 0) {
      resultParts.push(`❌ ${rejectedCount} ${rejectedCount === 1 ? 'salida rechazada' : 'salidas rechazadas'} (sin entrada)`);
    }
  ...
  const handleMarkAsAbsent = async () => {
    try {
      ...
      console.log('🔵 Marcando ausentes para:', selectedWorkersList.map(w => `${w.nombre_completo} (ID: ${w.trabajador_id})`));
      ...
      for (const worker of selectedWorkersList) {
        try {
          console.log(`📤 Enviando petición para ${worker.nombre_completo} (ID: ${worker.trabajador_id})`);
          
          // Llamar al nuevo endpoint mark-absent
          const response = await fetch(`${API_URL}/api/attendance/mark-absent`, {
            method: 'POST',
            headers: {
          ...
          if (!response.ok) {
            ...
            console.error(`❌ Error en respuesta para ${worker.nombre_completo}:`, errorData);
      ...
      console.log(`📊 Registros con deleted_at:`, result?.data?.filter((r: any) => r.deleted_at).length);
      console.log(`📋 Primeros 5 registros:`, result?.data?.slice(0, 5));

      toast.success(`✅ ${successCount} ${successCount === 1 ? 'trabajador marcado' : 'trabajadores marcados'} como ausente`);
  ...
  const { selectedDateAttendances, entriesCount, exitsCount, absentsCount } = useMemo(() => {
    const attendancesForCalc = Array.isArray(attendances) ? attendances : [];
    console.log(`📊 useMemo recalculando con ${attendancesForCalc.length} registros para fecha ${selectedDate}`);
    
    const dateSel = attendancesForCalc.filter((a) => getDateString(a.fecha_at) === selectedDate);
    ...
    console.log(`✅ ${activeRecords.length} activos, ❌ ${absentRecords.length} ausentes`);
    ...
    const activeWorkerIds = new Set(activeRecords.map(a => a.trabajador_id));
    ...
    console.log(`📈 Contadores: Entradas=${entryWorkerIds.size}, Salidas=${exitWorkerIds.size}, Ausentes=${absentWorkerIds.size}`);
  ...
  const handleBulkRegister = async (type: 'entrada' | 'salida', time: string, location?: string) => {
    
    const dateToCheck = selectedDate;
    const attendancesArray = Array.isArray(attendances) ? attendances : [];
    const selectedDateAttendances = attendancesArray.filter((a) => getDateString(a.fecha_at) === dateToCheck);
    ...
    if (type === 'entrada') {
      ...
      for (const worker of workers) {
        ...
        if (activeRecord && activeRecord.hora_entrada_at) {
          ...
          console.log(`  🔁 ${worker.nombre_completo}: Reactivar desde ausencia`);
      ...
      for (const worker of workers) {
        ...
        if (!activeRecord) {
          console.log(`  ⛔ ${worker.nombre_completo}: Sin entrada (rechazado)`);
        ...
        if (exitTotalMinutes <= entryTotalMinutes) {
          console.log(`  ⚠️ ${worker.nombre_completo}: Salida antes que entrada`);
          invalidTimeWorkers.push(worker);
      ...
      if (invalidTimeWorkers.length > 0) {
        const namesList = invalidTimeWorkers.map(w => `  • ${w.nombre_completo}`).join('\n');
    ...
    if (hasAnyEntryInDate) {
      console.log(`  📋 MOSTRANDO CONFIRMACIÓN porque existen entradas previas`);
      if (type === 'entrada') {
        setPendingBulkEntryData({
    ...
    if (type === 'entrada') {
      ...
      executeBulkRegister(registeredWorkers, allToUpdate, toRejectWorkers, type, time, location || '');
    } else {
      executeBulkRegister(registeredWorkers, toUpdateWorkers, toRejectWorkers, type, time, '');
    }
  };
  ...
  const executeBulkRegister = async (
    ...
    try {
      ...
      if (registered.length > 0) {
        resultParts.push(`✅ ${registered.length} ${registered.length === 1 ? typeLabel + ' registrada' : typeLabel + 's registradas'}`);
      }
      if (toUpdate.length > 0) {
        resultParts.push(`🔄 ${toUpdate.length} ${toUpdate.length === 1 ? typeLabel + ' actualizada' : typeLabel + 's actualizadas'}`);
      }
      if (toReject.length > 0) {
        const rejectMessage = type === 'salida' 
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-slate-900 rounded-lg p-5 border border-slate-800">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-3xl font-bold text-white mb-1">{totalWorkers}</p>
        ...
        <div className="bg-slate-900 rounded-lg p-5 border border-slate-800">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-3xl font-bold text-white mb-1">{entriesCount}</p>
              <p className="text-gray-400 text-xs">Entradas Registradas</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-lg p-5 border border-slate-800">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-3xl font-bold text-white mb-1">{exitsCount}</p>
              <p className="text-gray-400 text-xs">Salidas Registradas</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-lg p-5 border border-slate-800">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-3xl font-bold text-white mb-1">{absentsCount}</p>
      ...
      <div className="flex gap-3 mb-6">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-900 border border-slate-800 text-white text-sm rounded-lg placeholder-gray-500 focus:outline-none focus:border-blue-500 transition-colors"
          />
        </div>
        <button
      ...
      {showBulkModal && (() => {
        ...
        const workersWithoutEntryCount = workers.length - workersWithEntryCount;
      ...
      {showBulkExitModal && (() => {
        ...
        const workersWithoutEntry = selectedWorkers.filter(w => 
          !attendancesArray.some(a => 
          ...
          <div className="bg-[#1a1f2e] border border-gray-800 rounded-lg p-8 max-w-md w-full">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl font-bold text-white">
                {pendingBulkExitData?.toUpdate?.length > 0 ? '¿Reemplazar salida?' : '¿Registrar salida?'}
              </h2>
            </div>

            <div className="mb-6 p-4 bg-yellow-900/20 border border-yellow-600/30 rounded-lg">
              <p className="text-sm text-yellow-300">
                {pendingBulkExitData?.registered?.length > 0 && `✅ ${pendingBulkExitData?.registered?.length} ${pendingBulkExitData?.registered?.length === 1 ? 'trabajador' : 'trabajadores'} registrará salida`}
                {pendingBulkExitData?.registered?.length > 0 && pendingBulkExitData?.toUpdate?.length > 0 && ' | '}
                {pendingBulkExitData?.toUpdate?.length > 0 && `🔄 ${pendingBulkExitData?.toUpdate?.length} ${pendingBulkExitData?.toUpdate?.length === 1 ? 'trabajador' : 'trabajadores'} reemplazará salida`}
              </p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              >
                {pendingBulkExitData?.toUpdate?.length > 0 ? 'Sí, Reemplazar' : 'Sí, Registrar'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-[#1a1f2e] border border-gray-800 rounded-lg p-8 max-w-md w-full">
            <div className="flex items-center gap-3 mb-6">
              <AlertCircle className="w-6 h-6 text-yellow-500" />
              <h2 className="text-xl font-bold text-white">
                {pendingBulkEntryData?.toUpdate?.length > 0 ? '¿Reemplazar entrada?' : '¿Registrar entrada?'}
              </h2>
            </div>
            ...
            <div className="mb-6 p-4 bg-yellow-900/20 border border-yellow-600/30 rounded-lg">
              <div className="space-y-2 text-sm text-yellow-300">
                {pendingBulkEntryData?.registered?.length > 0 && (
                  <p>
                    ✅ {pendingBulkEntryData?.registered?.length} {pendingBulkEntryData?.registered?.length === 1 ? 'trabajador registrará' : 'trabajadores registrarán'} entrada
                  </p>
                )}
                {pendingBulkEntryData?.toUpdate?.length > 0 && (
                  <p>
                    🔄 {pendingBulkEntryData?.toUpdate?.length} {pendingBulkEntryData?.toUpdate?.length === 1 ? 'trabajador' : 'trabajadores'} actualizará entrada
                  </p>
                )}
                {pendingBulkEntryData?.toReactivate?.length > 0 && (
                  <p>
                    🔁 {pendingBulkEntryData?.toReactivate?.length} {pendingBulkEntryData?.toReactivate?.length === 1 ? 'trabajador' : 'trabajadores'} será reactivado{pendingBulkEntryData?.toReactivate?.length === 1 ? '' : 's'} desde ausencia
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  if (pendingBulkEntryData) {
                    ...
                    const allToUpdate = [...(pendingBulkEntryData.toUpdate || []), ...(pendingBulkEntryData.toReactivate || [])];
                    processBulkEntry(
              ...
              >
                {pendingBulkEntryData?.toUpdate?.length > 0 ? 'Sí, Reemplazar' : 'Sí, Registrar'}
              </button>
            </div>
          </div>
        </div>
